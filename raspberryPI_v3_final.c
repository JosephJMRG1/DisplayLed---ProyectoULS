...
/* funcion maneja las senales recibidas del usuario */
...
void controlarLeds(int matriz[FILCOL][FILCOL], int tiempo)
{
...
void animarMatrices()
{
    while (!signalCatch) // Loop hasta que el usuario decida cancelar
    {
        ...
        while (fscanf(archivo, "%*[^\n]") != EOF) // Avanzar hasta el final de la linea
        {
            leerArchivo(matriz);
            printf("\nControlando LEDs segun la matriz...\n");
            controlarLeds(matriz,10);
            pausa();
...
int main()
{
    // Funcion para que el usuario cancele el programa
